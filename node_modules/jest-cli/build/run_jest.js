...
function _load_chalk() {return _chalk = _interopRequireDefault(require('chalk'));}var _path;
function _load_path() {return _path = _interopRequireDefault(require('path'));}var _jestUtil;
function _load_jestUtil() {return _jestUtil = require('jest-util');}var _exit;
function _load_exit() {return _exit = _interopRequireDefault(require('exit'));}var _gracefulFs;
...
function _load_test_result_helpers() {return _test_result_helpers = require('./test_result_helpers');}var _failed_tests_cache;
function _load_failed_tests_cache() {return _failed_tests_cache = _interopRequireDefault(require('./failed_tests_cache'));}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}function _asyncToGenerator(fn) {return function () {var gen = fn.apply(this, arguments);return new Promise(function (resolve, reject) {function step(key, arg) {try {var info = gen[key](arg);var value = info.value;} catch (error) {reject(error);return;}if (info.done) {resolve(value);} else {return Promise.resolve(value).then(function (value) {step("next", value);}, function (err) {step("throw", err);});}}return step("next");});};} /**
...
const processResults = (runResults, options) => {const
  ...
  {let contexts = _ref2.contexts,globalConfig = _ref2.globalConfig,outputStream = _ref2.outputStream,testWatcher = _ref2.testWatcher,startRun = _ref2.startRun,changedFilesPromise = _ref2.changedFilesPromise,onComplete = _ref2.onComplete,failedTestsCache = _ref2.failedTestsCache;
    const sequencer = new (_test_sequencer || _load_test_sequencer()).default();
    let allTests = [];

    if (changedFilesPromise && globalConfig.watch) {var _ref4 =
      yield changedFilesPromise;const repos = _ref4.repos;
    ...
    contexts.map((() => {var _ref5 = _asyncToGenerator(function* (context) {
    ...
    if (globalConfig.listTests) {
      const testsPaths = Array.from(new Set(allTests.map(function (test) {return test.path;})));
    ...
    if (!allTests.length) {
      const noTestsFoundMessage = (0, (_get_no_test_found_message || _load_get_no_test_found_message()).default)(
      testRunData,
      globalConfig);
